    ...
    async def roulette(self, ctx: commands.Context, player: discord.User | str = None):
        ...
        if player and player != bot:
            ...
            if answer.content == 'y':
                ...
                await ctx.send(embed = discord.Embed(description = translate(locale, "roulette_cancel"), color = 0x2f3136))
            ...
            for i in range(11):
                ...
                if p1_hp <= 0 or p2_hp <= 0:
                    await channel.send(embed = discord.Embed(description = f'{translate(locale, "roulette_winner")}'.format(winner = winner.mention), color = 0x00ff00 if winner == ctx.author.mention else 0xff0000))
                    with open('leaderboard/leaders.json', 'r') as file:
                        data = json.load(file)
