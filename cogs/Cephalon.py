...
class Cephalon(commands.Cog):
    ...
    async def help(self, ctx, command = None):
        ...
        #    await ctx.send('```python\ncy/volume <громкость>\nГромкость должна быть в пределе от 0 до 100\n\n<> - обязательно```')
            ...
            await ctx.send('```python\ncy/roulette [@пинг/имя/ID]\n\nСигарета - восстанавливает одно очко здоровья\nНожовка по металлу - следующий патрон нанесёт в 2 раза больше урона\nПиво - вы передёргиваете затвор\nЛупа - посмотреть, какой патрон сейчас в патроннике\nСтарый телефон - ответивший подскажет, на какой позиции находится боевой патрон\nПодозрительные таблетки - с 50% шансом вы можете получить 2 очка здоровья или потерять одно\nСтранная кассета - текущий патрон меняется на противоположный: боевой -> холостой, холостой -> боевой\nШприц адреналина - вы можете выбрать один предмет противника, кроме самого шприца\nИмейте ввиду, что при начале игры чат, в котором вы начали играть будет сильно засорён\n\n[] - опционально```')
