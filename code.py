...
async def about(ctx, member:discord.Member = None, amount = 1):
    ...
    emb.add_field(name = f'Роли [{len(member.roles)}]', value=' '.join([role.mention for role in member.roles[1:]]))
    emb.add_field(name = 'Высшая Роль', value = member.top_role.mention)
