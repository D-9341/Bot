...
async def give(ctx, member: discord.Member, *, role: discord.Role):
    ...
    if role is not None:
        if role > member.top_role and ctx.message.author.id != 338714886001524737:
            ...
            emb = discord.Embed(colour = member.color, timestamp = ctx.message.created_at)
            emb.add_field(name = 'Была выдана роль', value = f'{role.mention} || {role.name} || {role.id}')
...
async def take(ctx, member: discord.Member, *, role: discord.Role):
    ...
    if role is not None:
        if role > member.top_role and ctx.message.author.id != 338714886001524737:
            ...
            channel = client.get_channel(714175791033876490)
            emb = discord.Embed(colour = member.color, timestamp = ctx.message.created_at)
            emb.add_field(name = 'Была забрана роль', value = f'{role.mention} || {role.name} || {role.id}')
...
async def on_member_remove(member):
    ...
    if member.bot == False:
        ...
        emb = discord.Embed(description = f'{member.mention} ({member.name}), ну и с `{member.guild.name}` вали, ботаря, хаха!', colour = discord.Color.orange())
        emb.set_footer(text = 'Cephalon Cy by сасиска#2472')
        await channel.send(embed = emb)
    elif member.bot != False and member.id == 694170281270312991:
